...
def compute_lvmi(dicomdir, videofile, lvlength, hr, ft, window, x_scale, y_scale, nrow, ncol):
    ...
    psax_outer_segs = np.load(npydir + "/" + videofile + "_psaxo.npy")
    psax_outer_segs = remove_periphery(psax_outer_segs)
    psax_inner_segs = np.load(npydir + "/" + videofile + "_psaxi.npy")
    ...
    outer_area = rolling_median(pd.DataFrame(psax_outer_areas)[0], window=4, center=True).fillna(method='bfill').fillna(method='ffill').tolist()
    inner_area = rolling_median(pd.DataFrame(psax_inner_areas)[0], window=4, center=True).fillna(method='bfill').fillna(method='ffill').tolist()
    massmin = 25
    massmax = 500
    masslist = []
...
def compute_la_lv_volume(dicomdir, videofile, hr, ft, window, x_scale, y_scale, nrow, ncol):
    npydir = dicomdir + "/unet/"
    ...
    lv_areas = rolling_median(pd.DataFrame(lv_areas)[0], window=4, center=True).fillna(method='bfill').fillna(method='ffill').tolist()
    la_areas = extract_areas(la_segs)
    la_areas = rolling_median(pd.DataFrame(la_areas)[0], window=4, center=True).fillna(method='bfill').fillna(method='ffill').tolist()
    eflist = []
    efmax, efmin = 0.8, 0.1
...
def main():
    ...
    for videofile in viewlist_a4c + viewlist_a2c:
        ...
        lavol, lvedv, lvesv, ef, diasttime, lveda_l = compute_la_lv_volume(dicomdir, videofile, hr, ft, window, x_scale, y_scale, nrow, ncol)
